...
import web.admin.rejectmail.service.*;
...
public class RejectMailController extends MultiActionController{
	...
	public void setRejectMailService(RejectMailService rejectMailService){
	...
	public ModelAndView main(HttpServletRequest req, HttpServletResponse res) throws Exception{		
		...
		return new ModelAndView("/admin/rejectmail/rejectmail.jsp");
	}
	...
	public ModelAndView write(HttpServletRequest req, HttpServletResponse res) throws Exception{	
		...
		return new ModelAndView("/admin/rejectmail/rejectmail_proc.jsp?method=write");		
	}
	...
	public ModelAndView list(HttpServletRequest req, HttpServletResponse res) throws Exception{		
		...
		//총카운트 		
		int iTotalCnt = rejectMailService.totalCountRejectMail(searchMap);
		req.setAttribute("iTotalCnt", Integer.toString(iTotalCnt));

		req.setAttribute("message", this.message);
		this.message = ""; // 다음 호출을 위해 초기화
		
		return new ModelAndView("/admin/rejectmail/rejectmail_proc.jsp?method=list","rejectmailList", rejectMailService.listRejectMail(curPage, iLineCnt, searchMap));
	}
	...
	public ModelAndView delete(HttpServletRequest req, HttpServletResponse res) throws Exception{		
		...
		int[] resultVal = rejectMailService.deleteRejectMail(maps);
	...
	public ModelAndView insertTypeDirect(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		for(int i =0;i<emails.length;i++)
		{
			...
			if(emails[i].indexOf("\r\n") != -1){
				...
				rejectMail.setEmail(emails[i].split("\r\n")[0]);
			}
			else if(emails[i].indexOf(",") != -1){
				
				rejectMail.setEmail(emails[i].split(",")[0]);
			}
			else
				rejectMail.setEmail(emails[i]);
			rejectMail.setMassmailID(0);
			rejectMail.setMassmailGroupID(massmailGroupID);
			rejectMail.setTargetID(0);
			rejectMail.setUserID(userID);
			rejectMail.setGroupID(groupID);
			
			if(rejectMail.getEmail().indexOf("@")!= -1 && rejectMail.getEmail().indexOf(".")!= -1)
				resultVal = resultVal+rejectMailService.insertRejectMail(rejectMail);
		}
		
		if(resultVal!=emails.length){
	...
	public ModelAndView insertTypeFile(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		//업로드된 파일일 처리할 스레드 가동 
		RejectMailCSVThread rejectMailCSVThread = new RejectMailCSVThread(rejectMailService,massmailGroupID,userID,groupID,filePath,fileType);
		rejectMailCSVThread.start();
