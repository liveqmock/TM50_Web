...
<div class="search_wrapper" style="width:850px">
	<div class="start">
				<ul id="sSearchType"  class="selectBox" >
					...
					<li data="r.email">이메일</li>		
					<li data="i.massmailTitle">대량메일명</li>
					<li data="g.massmailGroupName">대량메일그룹명</li>
...
<div style="clear:both;width:850px">
	...
	<thead>
		...
		<th style="height:30px;width:160px">대량메일명</th>
		<th style="height:30px;width:160px">대량메일그룹명</th>
		<th style="height:30px;width:100px">작성자</th>
		<th style="height:30px;width:90px">등록구분</th>
...
$('<%=id%>').saveData = function() {
	...
	if(frm.eInputType[0].checked == true) {
		goUrl = 'admin/rejectmail/rejectmail.do?id=<%=id%>&method=insertTypeDirect';
	} else if(frm.eInputType[1].checked == true) {
		goUrl = 'admin/rejectmail/rejectmail.do?id=<%=id%>&method=insertTypeFile';
	}
...
$('<%=id%>').chgInputType = function(seq){
	...
	if(seq=='1'){
		...
		$('<%=id%>_email').setStyle('display','block');
	}
	//파일업로드이라면 
	else if(seq=='2'){
		$('<%=id%>_file').setStyle('display','block');
		$('<%=id%>_email').setStyle('display','none');
	}
};
...
$('<%=id%>').renderUpload = function() {
 ...
 $('<%=id%>').uploader = new SwFileUpload('<%=id%>Upload', {
  ...
  limitSize:50,
  uploadPage: 'admin/rejectmail/rejectmail.do?method=uploadFile',
...
$('<%=id%>').createPopup = function() {
	...
	$('<%=id%>').popup.add('삭제', function(target,e) { 
			$('<%=id%>').deleteData($('<%=id%>').grid_content.getSelectedRow().getAttribute("rejectmail_id") ); 
		}
	);
...
$('<%=id%>').deleteSelectedData = function(  ) {
	...
	nemoRequest.init( 
	{
		...
		, url: 'admin/rejectmail/rejectmail.do?method=delete&id=<%=id%>'
		, update: $('<%=id%>_grid_content') // 완료후 content가 랜더링될 element
		, onSuccess: function(html,els,resHTML) {
...
$('<%=id%>').deleteData = function( seq ) {
	...
	nemoRequest.init( 
	{
		...
		, url: 'admin/rejectmail/rejectmail.do?method=delete&id=<%=id%>&eRejectID=' + seq
		, update: $('<%=id%>_grid_content') // 완료후 content가 랜더링될 element
		, onSuccess: function(html,els,resHTML) {
			// 수정창을 닫는다
	...
	nemoWindow(
		{
			...
			type: 'modal',
			loadMethod: 'xhr',
			contentURL: 'admin/rejectmail/rejectmail.do?id=<%=id%>&method=write'
		}
	);
...
$('<%=id%>').list = function ( forPage ) {	
	...
	nemoRequest.init( 
	{
		...
		url: 'admin/rejectmail/rejectmail.do?method=list&id=<%=id%>', 
		update: $('<%=id%>_grid_content'), // 완료후 content가 랜더링될 element
...
$('<%=id%>').personPreviewDown = function() {
	...
	location.href = "admin/rejectmail/rejectmail_down.jsp?searchType="+searchType+"&searchText="+searchText;
}
