...
<div class="search_wrapper" style="width:650px">
	<div class="start">
		<ul id="sSearchType"  class="selectBox" >
			<li data="massMailGroupName" >그룹명</li>
...
<div style="clear:both;width:650px">
	...
	<thead>
		<tr>
		<th style="height:30px;width:50px"><input id="sCheckAll" class="notBorder" name="sCheckAll" type="checkbox" onclick="selectAll($('<%=id%>_list_form').eMassMailGroupID,this.checked)"/></th>
		<th style="height:30px;width:200px">그룹명</th>
...
$('<%=id%>').createPopup = function() {
	//alert($('<%=id%>').grid_content.getSelectedRow().getAttribute("massmailgroup_id").eMassMailGroupID);
	//if($('<%=id%>').grid_content.getSelectedRow().getAttribute("massmailgroup_id") != 1)
	//{
		$('<%=id%>').popup = new PopupMenu();

		$('<%=id%>').popup.add('수정', 
			function(target,e) {
				$('<%=id%>').editWindow( $('<%=id%>').grid_content.getSelectedRow().getAttribute("massmailgroup_id") );
			}
		);
		...
		$('<%=id%>').popup.add('삭제', function(target,e) { 
						$('<%=id%>').deleteData($('<%=id%>').grid_content.getSelectedRow().getAttribute("massmailgroup_id") ); 
					}
		);
	...
	nemoWindow(
		{
			...
			height: 200,
			title: '대량 메일 그룹 등록 / 수정',
			type: 'modal',
			loadMethod: 'xhr',
			contentURL: 'admin/massmailgroup/massmailgroup.do?id=<%=id%>&method=edit&massMailGroupID='+seq
		}
	);
...
$('<%=id%>').saveData = function( seq ) {
	...
	if(seq == '0') {
		goUrl = 'admin/massmailgroup/massmailgroup.do?id=<%=id%>&method=insert';
	} else {
		goUrl = 'admin/massmailgroup/massmailgroup.do?id=<%=id%>&method=update';
	}
...
$('<%=id%>').deleteSelectedData = function(  ) {
	...
	var checked = isChecked( frm.elements['eMassMailGroupID']  );
	...
	if(frm.elements['eMassMailGroupID'].length == checked) {
	...
	nemoRequest.init( 
	{
		...
		, url: 'admin/massmailgroup/massmailgroup.do?method=delete&id=<%=id%>'
		, update: $('<%=id%>_grid_content') // 완료후 content가 랜더링될 element
		, onSuccess: function(html,els,resHTML) {
...
$('<%=id%>').deleteData = function( seq ) {
	...
	nemoRequest.init( 
	{
		...
		, url: 'admin/massmailgroup/massmailgroup.do?method=delete&id=<%=id%>&eMassMailGroupID=' + seq
		, update: $('<%=id%>_grid_content') // 완료후 content가 랜더링될 element
		, onSuccess: function(html,els,resHTML) {
			// 수정창을 닫는다
...
$('<%=id%>').list = function ( forPage ) {
	...
	nemoRequest.init( 
	{
		...
		url: 'admin/massmailgroup/massmailgroup.do?method=list&id=<%=id%>', 
		update: $('<%=id%>_grid_content'), // 완료후 content가 랜더링될 element
