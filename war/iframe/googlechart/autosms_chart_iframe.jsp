...
<%@ page import="web.autosms.model.AutoSMSStatistic"%>
...
<% 
	...
	int autosmsID = new Integer(request.getParameter("autosmsID"));
      ...
      function drawChart() { 
        ...
        <%
        	...
        	for(AutoSMSStatistic mailStatistic : mailStatisticList){%>
        		...
        		data.setValue(<%=i%>, 2, <%=mailStatistic.getReadyTotal()%>);
    	        data.setValue(<%=i%>, 3, <%=mailStatistic.getSuccessTotal()%>);
    	        data.setValue(<%=i%>, 4, <%=mailStatistic.getFailTotal()%>); 
