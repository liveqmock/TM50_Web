...
<jsp:useBean id="pollTemplateID" class="java.lang.String" scope="request" />
<%	
	//templateID를 통해 ez_massmail_templates에 templateContent를 가져온다. 
	pollTemplateID = request.getParameter("pollTemplateID") == null ? "0" : request.getParameter("pollTemplateID").trim();

	PollTemplate pollTemplate = null;
	//모델을 이용하여 리스트를 가져온다.
	if(!pollTemplateID.equals("") && !pollTemplateID.equals("0")){
		PollService service = PollControlHelper.getUserService(application);
		pollTemplate = service.viewPollTemplate(Integer.parseInt(pollTemplateID));
	}
	...
	if(pollTemplate!=null){
		fckEditor.setValue(pollTemplate.getPollTemplateHTML());
	}
