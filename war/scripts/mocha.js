...
var MochaUI = new Hash({
	options: new Hash({
		useEffects: false //false //(Browser.Engine.trident)? false: true  // Toggles the majority of window fade and move effects.
	}),
	Columns: {
		instances:      new Hash()
	},
			...
			case 'xhr':
				op = {
					...
					onRequest: function(){
						...
						// nemo 2008-11-11 
						showBusy(currentInstance.contentBorderEl);
						// nemo 2008-11-11  contentWrapperEl contentBorderEl
						//alert('1');
 
					}.bind(this),
					// nemo 2008-11-11 
					onComplete: function() {
						//hideBusy(currentInstance.contentBorderEl);
					},
					// nemo 2008-11-11 
					onFailure: function(){
						//alert('2');
					...
					onSuccess: function(){
						...
						if (contentContainer == contentEl){
							...
							/* nemo add 2008-10-30 */
							//if(toolTip)	toolTip.init($(element.id));
				...
				if(Browser.Engine.trident4)
					$extend(op,{async : false } );
				new Request.HTML(op).get();
				break;
	...
	*/
	...
	closingJobs: function(windowEl){
		...
		/* NEMO 2008-12-24 */
		/* 모달의 모달을 닫으면 Z-INDEX 가 변하면 안됨 */
		if (currentInstance.options.type != 'notification' &&  
			(!(currentInstance.options.type == 'modal' || currentInstance.options.type == 'modal2') ||   MochaUI.modalCount == 0     )){
...
MochaUI.Windows.windowOptions = {
	...
	useCanvas:         true,
	//useCanvas:         !Browser.Engine.trident4,
	useCanvasControls: true,
	//useCanvasControls: !Browser.Engine.trident4,
	...
	*/
	newWindow: function(properties){ // options is not doing anything
		...
		if ( Browser.Engine.trident && this.options.shape == 'gauge') {
			this.windowEl.setStyle('background', 'url(../images/spacer.gif) repeat');
		}
		...
		*/ //old
		...
		if (this.options.toolbar == true) {
			if(this.options.toolbarURL){ // nemo 2008-11-22
				MochaUI.updateContent({
					'element':       this.windowEl,
					'childElement':  this.toolbarEl,
					'content':       this.options.toolbarContent,
		...
		if (this.options.type == 'modal' || this.options.type == 'modal2') {
			...
			*/
			...
			if(MochaUI.modalCount == 1)
				$('modalOverlay').setStyle('display', 'block');
			if (MochaUI.options.useEffects == false){
				if(MochaUI.modalCount == 1)
					$('modalOverlay').setStyle('opacity', MochaUI.Modal.opacity);
				this.windowEl.setStyles({
					'zIndex': 11000,
					'opacity': 1
				});
	...
	setupEvents: function(windowEl) {
		...
		if (this.options.type == 'window'){
			windowEl.addEvent('mousedown', function(e) {
				...
				//if(Browser.Engine.name.test('trident')) // nemo 2008-11-22
				//	new Event(e).stop();
			}.bind(this));
		}
	...
	*/
	insertWindowElements: function(){
		...
		*/ //2008-11-27
		...
		//2008-12-12
		/*
		if(!this.useCanvas && Browser.Engine.trident4) {

			cache.statusBorderEl = new Element('div', {
				'id': id + '_statusBorder',
				'class': 'mochaStatusBorder',
				'styles' : {'width': '100%'}

			}).inject(cache.overlayEl);

			new Element('img', {
					'src': 'images/close_btn.gif'}).inject( cache.closeButtonEl );

			if(cache.se) {
				new Element('img',{
					'src': 'images/resize_btn.gif'}).inject( cache.se );

				cache.se.setStyle('opacity',1);
			}
			
			//cache.titleBarEl.setStyle('background-image','url(images/title_back.gif)');
			cache.titleBarEl.setStyle('background-color','#fff');
			cache.titleBarEl.setStyle('border','1px solid #C3C3C3');
			
			cache.contentBorderEl.setStyle('border-top','0');
			cache.contentBorderEl.setStyle('border-bottom','0');

			cache.contentBorderEl.setStyle('border-left','1px solid #C3C3C3');
			cache.contentBorderEl.setStyle('border-right','1px solid #C3C3C3');

			if( $defined(cache.toolbarEl) ) {
				cache.toolbarWrapperEl.setStyle('border-left','1px solid #C3C3C3');
				cache.toolbarWrapperEl.setStyle('border-right','1px solid #C3C3C3');
				cache.toolbarWrapperEl.setStyle('border-bottom','1px solid #C3C3C3');
				cache.toolbarWrapperEl.setStyle('background-color','#F3F3F3');
			}

			cache.titleBarEl.setStyle('filter','progid:DXImageTransform.Microsoft.gradient(StartColorStr=#FFFFFF,EndColorStr=#C3C3C3)');

			
			cache.shadowEl = new Element('div', {
				'id': id + '_shadow',
				'styles' : {
					'position' : 'absolute',
					'background-color': 'white'
				}
			}).inject(this.windowEl,'top');
...
MochaUI.Modal = new Class({
	...
	setModalOpacity: function( opacity ) {
		this.opacity = opacity;
	},
	setModalBackgroundColor: function( color ) {
		$('modalOverlay').setStyle('background-color', color);
...
MochaUI.options.extend({
	...
	viewportTopOffset:  -10,    // Use a negative number if neccessary to place first window where you want it
	viewportLeftOffset: -10,
	windowTopOffset:    20,    // Initial vertical spacing of each window
	windowLeftOffset:   20     // Initial horizontal spacing of each window	
	...
	*/
	initializeTabs: function(el){
		...
		if(Browser.Engine.trident4) {
			...
			$(el).getParent().set('class','toolbarTabsIE6');
			$(el).set('class','tab-menuIE6');

		} else {

			$(el).set('class','tab-menu');
		...
		*/
		...
		$(el).getElements('li').each(function(listitem){
			//listitem.setStyle('display','inline');
...
Arguments:
	...
	*/
	selected: function(el, parent){
		...
		if($(parent).getParent().getAttribute('is_multi') == 'Y') {

			var root = $(parent).getParent().getParent();

			root.getChildren().each( function( parentEl ) {
		
				if('toolbarTabsIE6'.test(parentEl.className,'i')) {
					$(parentEl).getFirst().getChildren().each(function(listitem){
...
MochaUI.Desktop = new Class({
	...
	menuInitialize: function(){
		...
		if (Browser.Engine.trident4 && this.desktopNavBar){
			...
			// nemo 2008-11-09 start
			/* 2008-11-27
			$$('.returnFalse').each( function(el) {  
				
					pa = el.getParent();

					frm = new Element('iframe', {
							'src': '',
							'marginwidth':  0,
							'marginheight': 0,
							'frameBorder':  0,
					...
					}).inject( pa.getChildren()[1], 'top' ); 

					frm.setStyle( 'width',  (pa.getChildren()[1].offsetWidth -4 ) +'px');
					frm.setStyle( 'height', (pa.getChildren()[1].offsetHeight  )+'px');
	...
	setDesktopSize: function(){
		...
		if (this.pageWrapper) {
			// nemo 2008-11-28
			var dockOffset = 0;
			if(!Browser.Engine.trident4 && dockWrapper) {
				dockOffset = MochaUI.dockVisible ? dockWrapper.offsetHeight : 0;
			}
	...
	*/	
	maximizeWindow: function(windowEl){
		...
		currentInstance.maximizeButtonEl.setProperty('title', '이전크기로');
		MochaUI.focusWindow(windowEl);
	...
	*/	
	restoreWindow: function(windowEl){	
		...
		if (options.restrict){
			...
			currentInstance.titleBarEl.setStyle('cursor', '이동');
		}		
		...
		currentInstance.maximizeButtonEl.setProperty('title', '최대화');
	}
});
...
MochaUI.Column = new Class({
	...
	options: {
		...
		placement:     null, 
		width:         null,
		parentObj:		   null,  // nemo 2009-03-24 
		resizeLimit:   [],
	...
	initialize: function(options){
		...
		*/ // nemo 2009-03-24 
		...
		if(options.parentObj)
			this.columnEl.inject(options.parentObj)
		else
			this.columnEl.inject($(MochaUI.Desktop.pageWrapper));
