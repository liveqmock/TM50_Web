...
var MochaUI = new Hash({
	...
	*/
	closeWindow: function(windowEl){
		...
		var instances = MochaUI.Windows.instances;
		var currentInstance = instances.get(windowEl.id);
		if(currentInstance == null) return; // nemo create
	...
	*/
	newWindow: function(properties){ // options is not doing anything
		...
		if (this.options.toolbar == true){
			if(this.options.toolbarURL){ // nemo create
				MochaUI.updateContent({
					'element':       this.windowEl,
	...
	*/
	insertWindowElements: function(){
		...
		if (this.options.useCanvas == true && Browser.Engine.trident) {
			...
			if ((Browser.Engine.trident && !Browser.ie9 )&& MochaUI.ieSupport == 'excanvas'){
...
MochaUI.Modal = new Class({
	...
	setModalOpacity: function( opacity ) { // nemo create
		this.opacity = opacity;
	},
	setModalBackgroundColor: function( color ) { // nemo create
...
MochaUI.Desktop = new Class({
	...
	setDesktopSize: function(){
		...
		if (this.pageWrapper) {
			...
			//var dockOffset = MochaUI.dockVisible ? dockWrapper.offsetHeight : 0;  // nemo close;
			//**** nemo create ****//
			var dockOffset = 0;
			...
			if(!Browser.Engine.trident4) // nemo create
	...
	*/	
	restoreWindow: function(windowEl){	
		...
		if (options.restrict){
			...
			currentInstance.titleBarEl.setStyle('cursor', '최대화');
		}		
		...
		currentInstance.maximizeButtonEl.setProperty('title', '�ִ�ȭ');
	}
});
...
MochaUI.Column = new Class({
	...
	options: {
		...
		parentObj:		   null,  // nemo create 2009-03-24 
