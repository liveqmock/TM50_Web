...
Mif.Tree = new Class({
	...
	initialize: function(options) {
		...
		this.wrapper=new Element('div').addClass('mif-tree-wrapper').injectInside(this.container.empty());
		this.initEvents();
		this.initScroll();
		this.initSelection();
		this.initHover();
		if (MooTools.version >= '1.2.2' && this.options.initialize) this.options.initialize.call(this); // modi 2013-03-18
	},
	...
	scrollTo: function(node){
		...
		//if(position==-1 || ( !up && !down ) ) { 2009-04-06 // nemo
		if(position==-1 ) {
...
Mif.Tree.Node = new Class({
	...
	getDOM: function(what){
		...
		if(!wrapper) return null;  // 2009-05-20 nemo
...
Mif.Tree.Hover={
	...
	over: function(node, target){
		...
		if(!wrapper) return; // nemo 2009-05-20
...
Mif.Tree.implement({
	...
	load: function(options){
		...
		function success(json){
			...
			tree.$getIndex();
			//tree.fireEvent('load'); // 2009-05-20 nemo ��
			options.onLoad(); // 2009-05-20 nemo
			return tree;
		}
...
Mif.Tree.Drag = new Class({
	...
	initialize: function(tree, options){
		...
		if(tree.options.logEl) this.logEl = tree.options.logEl;
		...
		this.cancelDrag = false; /* nemo 2009-05-28 */

		this.draging = false; /* nemo 2009-06-01 */
		...
		this.addEvent('start', function(){
			...
			//if(Mif.Tree.Drag.current) { // nemo 2009-05-04
	...
	leave: function(event){
		...
		this.draging = false; /* nemo 2009-06-01 */
		//if(this.logEl) this.logEl.appendText('leave<br/>');
	},
	...
	enter: function(event, zone){
		...
		//if(this.logEl) this.logEl.appendText('enter<br/>');
	},
	...
	autoScroll: function(){
		...
		//if(this.logEl) this.logEl.appendText('autoScroll<br/>');
	},
	...
	start: function(event){//mousedown
		...
		//if(this.logEl) this.logEl.appendText('start<br/>');
	},
	
	drag: function(event){
		/*
		if(!Mif.Tree.Drag.ghost) { // 2009-05-04 nemo
			this.removeEvents();
			this.onstop();
			Mif.Tree.Drag.current = null;
			return;
		}*/
		this.cancelDrag = false; /* nemo 2009-05-28 */
		this.draging = true; /* nemo 2009-06-01 */
		...
		//if(this.logEl) this.logEl.appendText('drag<br/>');
	...
	ondrag: function(event){
		...
		//if(this.logEl) this.logEl.appendText('ondrag<br/>');
	...
	emptydrop: function(){
			...
			if(this.options.animate){
				...
				Mif.Tree.Drag.ghost.set('morph',{
					...
					onComplete: function(){
						...
						//Mif.Tree.Drag.ghost = null; // 2009-05-20 nemo
		...
		this.draging = false; /* nemo 2009-06-01 */
		//if(this.logEl) this.logEl.appendText('emptydrop<br/>');
	},
	...
	drop: function(){
		...
		this.draging = false; /* nemo 2009-06-01 */
		//if(this.logEl) this.logEl.appendText('drop<br/>');
	},
...
Mif.Tree.Drag.Element=new Class({
	...
	drop: function(){
		Mif.Tree.Drag.ghost.dispose();
		//Mif.Tree.Drag.ghost = null; // nemo 2009-05-20
...
Mif.Tree.implement({
	...
	initGroupCheckbox: function(type){
	...
	checkboxClick: function(event){
		...
		// 2009-04-10 nemo
		this.fireEvent('checkboxclick',[this.mouse.node, (this.mouse.node.state.checked == 'checked')]);
	},
	...
	// 2009-04-10 nemo
	getCheckedCount: function(type) {
		var checkedCount = 0;
	    var test = new RegExp(type);
		this.root.recursive(function(){
			if(type) {
				if(test.test(this.type) && this.hasCheckbox && this.state.checked == 'checked') checkedCount ++;
			} else {
				if(this.hasCheckbox && this.state.checked == 'checked') checkedCount ++;
			}
		});
		return checkedCount;
	},
	...
	isChecked: function(type) {
		...
		this.root.recursive(function(){
			...
			if(type) {
				if(test.test(this.type) && this.hasCheckbox && this.state.checked == 'checked') {
					rslt = true;
					stop = true;
				}
			} else {
				if(this.hasCheckbox && this.state.checked == 'checked') {
					rslt = true;
					stop = true;
				}
			}
...
Mif.Tree.Node.implement({
	...
	initGroupCheckBox: function() {
		...
		var state = this.hasCheckbox? 'group-nochecked': 'nochecked';
		this.state.checked=state;
		if(!this.parentNode || (this.parentNode && this.parentNode.$draw)){
			this.getDOM('checkbox').removeClass('mif-tree-node-'+oldState).addClass('mif-tree-node-group-nochecked');
		}
	},

	//2009-04-09
	enableCheckBox: function( bool ) {

		var checked=(this.state.checked=='checked' && this.hasCheckbox) ? 'checked' : 'unchecked';
		var oldState=this.state.checked;
		this.hasCheckbox = bool;
		var state = (this.hasCheckbox)? ((checked=='checked')? 'checked' : 'unchecked'): 'nochecked';
		this.state.checked=state;
		if(!this.parentNode || (this.parentNode && this.parentNode.$draw)){
	...
	'switch' : function(state){
		if(!this.hasCheckbox || this.state.checked==state) return;
