...
<div style="clear:both;width:810px">
	...
	<thead>
		<tr>
		<th style="height:30px;width:50px"><input id="sCheckAll" class="notBorder" name="sCheckAll" type="checkbox" onclick="selectAll($('<%=id%>_list_form').ePollTemplateID,this.checked)"/></th>
		<th style="height:30px;">설문템플릿명</th>
		<th style="height:30px;width:100px">작성자</th>
		<th style="height:30px;width:100px">공유그룹</th>
		<th style="height:30px;width:80px">사용여부</th>
		<th style="height:30px;width:200px">등록일</th>
		</tr>
	</thead>
...
$('<%=id%>').createPopup = function() {
	...
	$('<%=id%>').popup.add('수정', 
		function(target,e) {
			$('<%=id%>').editWindow( $('<%=id%>').grid_content.getSelectedRow().getAttribute("polltemplate_id"), $('<%=id%>').grid_content.getSelectedRow().getAttribute("sharegroup_id") );
		}
	);
	...
	$('<%=id%>').popup.add('삭제', function(target,e) { 
			$('<%=id%>').deleteData($('<%=id%>').grid_content.getSelectedRow().getAttribute("polltemplate_id") ); 
		}
	);
	...
	nemoWindow(
		{
			...
			type: 'modal',
			loadMethod: 'xhr',
			contentURL: 'content/poll/poll.do?id=<%=id%>&method=editPollTemplate&ePollTemplateID='+seq+'&shareGroupID='+shareGroupID
		}
	);
...
$('<%=id%>').checkData = function( seq ) {
	var frm = $('<%=id%>_form');
	var goUrl = '';	
	frm.ePollTemplateHTML.value = $("<%=id%>_ifrmFckEditor").contentWindow.getFCKHtml();

	if(!frm.ePollTemplateHTML.value) {
	...
	copyForm( $('<%=id%>_rform'), frm );
		...
		goUrl = 'pages/content/poll/poll_template_proc.jsp?id=<%=id%>&method=check';
...
$('<%=id%>').saveData = function( seq ) {
	...
	if(seq == '0') {
		goUrl = 'content/poll/poll.do?id=<%=id%>&method=insertPollTemplate';
	} else {
		goUrl = 'content/poll/poll.do?id=<%=id%>&method=updatePollTemplate';
	}
...
$('<%=id%>').deleteSelectedData = function(  ) {
	...
	var checked = isChecked( frm.elements['ePollTemplateID']  );
	...
	if(frm.elements['ePollTemplateID'].length == checked) {
	...
	nemoRequest.init( 
	{
		...
		, url: 'content/poll/poll.do?method=deletePollTemplate&id=<%=id%>'
		, update: $('<%=id%>_grid_content') // 완료후 content가 랜더링될 element
		, onSuccess: function(html,els,resHTML) {
...
$('<%=id%>').deleteData = function( seq ) {
	...
	nemoRequest.init( 
	{
		...
		, url: 'content/poll/poll.do?method=deletePollTemplate&id=<%=id%>&ePollTemplateID=' + seq
		, update: $('<%=id%>_grid_content') // 완료후 content가 랜더링될 element
		, onSuccess: function(html,els,resHTML) {
			// 수정창을 닫는다
...
$('<%=id%>').list = function ( forPage ) {	
	...
	nemoRequest.init( 
	{
		...
		url: 'content/poll/poll.do?method=listPollTemplate&id=<%=id%>', 
		update: $('<%=id%>_grid_content'), // 완료후 content가 랜더링될 element
