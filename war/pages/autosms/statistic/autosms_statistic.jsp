...
$('<%=id%>').getData = function() {
	...
	{
		page = 'autosms/autosms.do?method=statisticHourlyList';
	}
    //일별 통계 처리
	if($('<%=id%>_sform').standard.value == 'daily')
	{
		page = 'autosms/autosms.do?method=statisticDailyList';
	}
    //월별 통계 처리
	if($('<%=id%>_sform').standard.value == 'monthly')
	{
		page = 'autosms/autosms.do?method=statisticMonthlyList';
	...
	nemoWindow(
			{
				...
				type: 'modal',
				loadMethod: 'xhr',
				contentURL: 'pages/autosms/statistic/autosms_statistic_proc.jsp?id=<%=id%>_preview&method=personpreview&method=personPreview&autosmsID=<%=autosmsID%>&sYear='+$('<%=id%>_sform').sYear.value+'&sDay='+$('<%=id%>_sform').sDay.value+'&sMonth='+$('<%=id%>_sform').sMonth.value+'&standard='+$('<%=id%>_sform').standard.value+'&type='+type+'&key='+key
			}
		);
}
/***********************************************/
...
$('<%=id%>').loadContent = function(packages) {
	...
	page = 'pages/autosms/statistic/autosms_statistic_proc.jsp?method=chartview&packages='+packages;
	nemoRequest.init( 
		{
			url: page, 
...
$('<%=id%>').excelDown = function() {
	var frm = $('<%=id%>_sform');
	location.href = "pages/autosms/statistic/autosms_statistic_excel.jsp?sStandard="+frm.standard.value+"&autosmsID="+frm.autosmsID.value+"&sYear="+frm.sYear.value+"&sMonth="+frm.sMonth.value+"&sDay="+frm.sDay.value;
}
