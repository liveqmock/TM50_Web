...
%>
	<!-- 자료가 하나도 없을경우 메인컨텐츠의 업데이트를 위하여 주석을 남긴다
		 ...
		 (이주석은 지우지 말것 반드시 필요한 주석임)!!!!!!!!!!!!!!!!! 
	-->
		<TABLE class="tbl" border="0" cellspacing="0" cellpadding="0" width="860px" align="center">
			<thead>
				<tr>
...
if(method.equals("chartview")) { 
	...
	if(standard.equals("failcause")){%>
		<IFRAME id="<%=id%>_ChartView" name="<%=id%>_ChartView" src="iframe/googlechart/auto_failcause_chart_iframe.jsp?automailID=<%=automailID%>&sYear=<%=sYear%>&sMonth=<%=sMonth %>&sDay=<%=sDay %>&packages=<%=packages%>&standard=<%=standard %>" width="840px" height="310px" scrolling=no frameborder=0 marginwidth=0 marginheight=0></IFRAME>
<%  }else{ %>
		<IFRAME id="<%=id%>_ChartView" name="<%=id%>_ChartView" src="iframe/googlechart/auto_chart_iframe.jsp?automailID=<%=automailID%>&sYear=<%=sYear%>&sMonth=<%=sMonth %>&sDay=<%=sDay %>&packages=<%=packages%>&standard=<%=standard %>" width="840px" height="310px" scrolling=no frameborder=0 marginwidth=0 marginheight=0></IFRAME>
<%  }
}
//****************************************************************************************************/
// 대상자 미리보 기
...
%>
	...
	$('<%=id%>').excelDown = function() {
		var frm = $('<%=id%>_sform');
		location.href = "pages/automail/statistic/automail_statistic_porsonpreview_excel.jsp?sStandard="+frm.standard.value+"&automailID="+frm.automailID.value+"&sYear="+frm.sYear.value+"&sMonth="+frm.sMonth.value+"&sDay="+frm.sDay.value+"&key="+frm.key.value+"&type="+frm.type.value+"&sSearchType="+frm.sSearchType.value+"&sSearchText="+frm.sSearchText.value;
	}
	/***********************************************/
	/* 리스트 
	/***********************************************/
	...
	$('<%=id%>').list = function ( forPage ) {
		...
		nemoRequest.init( 
		{
			...
			url: 'automail/automail.do?method=porsonPreview&id=<%=id%>', 
