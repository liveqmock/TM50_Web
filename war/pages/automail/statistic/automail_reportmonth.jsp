...
<br>
	...
	<thead>
		...
		<th style="height:30px;width:90px">오픈통수</th>
		</tr>
	</thead>
...
$('<%=id%>').list = function ( forPage ) {	
	...
	nemoRequest.init( 
	{
		...
		url: 'automail/automail.do?method=autoMailReportMonth&id=<%=id%>', 
...
$('<%=id%>').totalinfo = function () {	
	...
	nemoRequest.init( 
	{
		...
		url: 'automail/automail.do?method=autoMailReportMonthAll&id=<%=id%>', 
...
$('<%=id%>').excelDown = function () {	
	...
	location.href = "pages/automail/statistic/automail_reportmonth_excel.jsp?sSendedYear="+frm.sSendedYear.value+"&sSendedMonth="+frm.sSendedMonth.value+"&iTotalCnt="+frmList.iTotalCnt.value;
}
/* 리스트 표시 */
