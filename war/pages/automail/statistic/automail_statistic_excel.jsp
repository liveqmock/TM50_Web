...
<%
	...
	String fileName = "AutoMail_Report.xls";
    ...
    try {
    	 ...
    	 sheet = workbook.createSheet("자동메일 - "+title+"별 통계 현황" , 0);
    	...
    	sheet.mergeCells(0, 0, 4, 0);
 		label = new Label(0, 0, "자동메일 - "+title+"별 통계 현황" , titleFormat);
 		sheet.addCell(label);
        ...
        if(standard.equals("failcause")){
        	 for(FailCauseStatistic failCauselStatistic : failCauselStatisticList){
        		...
        		sheet.mergeCells(3, 2+i, 4, 2+i);
     	        ...
     	        sheet.addCell(label);
     	        number = new Number(2,2+i,failCauselStatistic.getFailCount().intValue(), dataFormat);
        	 ...
        	 for(MailStatistic mailStatistic : mailStatisticList){
                ...
                sheet.addCell(label);
                number = new Number(1,2+i,mailStatistic.getSendTotal(), dataFormat);
         	    sheet.addCell(number);
         	    number = new Number(2,2+i,mailStatistic.getSuccessTotal(), dataFormat);
         	    sheet.addCell(number);
         	    number = new Number(3,2+i,mailStatistic.getFailTotal(), dataFormat);
         	    sheet.addCell(number);
         	    number = new Number(4,2+i,mailStatistic.getOpenTotal(), dataFormat);
         	    sheet.addCell(number); 
        	 }
        }
