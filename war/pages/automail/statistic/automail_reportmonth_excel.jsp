...
<%@ page import="web.automail.service.*"%>
<%@ page import="web.automail.control.AutoMailControlHelper"%>
<%@ page import="web.automail.model.*"%>
...
<%	
	...
	List<MailStatistic> mailStatisticList = service.autoMailReportMonth(searchMap);
	MailStatistic mailStatistic = service.autoMailReportMonthAll(sendTime);
	
	out.clear();
	File file = (File)application.getAttribute("javax.servlet.context.tempdir");
	String filePath = file.getAbsolutePath();
	String fileName = "AutoMailReportMonth.xls";
    ...
    try {
        ...
        sheet.mergeCells(0, 0, 15, 0);
 		...
 		label = new Label(0,3,"총  메일발송량", defaultFormat);
        sheet.addCell(label);
        number = new Number(2,3,mailStatistic.getSendTotal(), dataFormat);
        sheet.addCell(number);
        

 		label = new Label(4,3,"성공통수", defaultFormat);
        sheet.addCell(label);
        number = new Number(6,3,mailStatistic.getSuccessTotal(), dataFormat);
        sheet.addCell(number);
        

 		label = new Label(8,3,"실패통수", defaultFormat);
        sheet.addCell(label);
        number = new Number(10,3,mailStatistic.getFailTotal(), dataFormat);
        sheet.addCell(number);
        

 		label = new Label(12,3,"오픈통수", defaultFormat);
        sheet.addCell(label);
        number = new Number(14,3,mailStatistic.getOpenTotal(), dataFormat);
        sheet.addCell(number);
        ...
        sheet.addCell(label);
 		...
 		label = new Label(14,6,"오픈통수", defaultFormat);
        sheet.addCell(label);
        
        int i = 0;
        
        for ( MailStatistic automail:mailStatisticList){
        	i++;
        	...
        	sheet.mergeCells(0, 6+i, 0, 6+i);
     		...
     		sheet.mergeCells(14, 6+i, 15, 6+i);
     		
     		label = new Label(0, 6+i, automail.getAutomailID()+"", dataFormat);
            sheet.addCell(label);
            label = new Label(1, 6+i, automail.getAutomailTitle(), dataFormat);
            sheet.addCell(label);
            label = new Label(6, 6+i, automail.getSendTimeHour(), dataFormat);
            sheet.addCell(label);
            label = new Label(8, 6+i, automail.getSendTotal()+"", dataFormat);
            sheet.addCell(label);
            label = new Label(10, 6+i, automail.getSuccessTotal()+"", dataFormat);
            sheet.addCell(label);
            label = new Label(12, 6+i, automail.getFailTotal()+"", dataFormat);
            sheet.addCell(label);
            label = new Label(14, 6+i, automail.getOpenTotal()+"", dataFormat);
            sheet.addCell(label);
        }
		...
		System.out.println("Excel : " + e.toString());
	} finally {
