	...
	<form id="<%=id%>_sform" name="<%=id%>_sform">
		...
		<div style="clear:both;width:97%">
			...
			<thead>
				...
				<th style="height:30px;width:150px">테스트폰번호</th>
				<th style="height:30px;width:50px">전송여부</th>
				<th style="height:30px;">발송결과</th>
		...
		$('<%=id%>').list = function ( forPage ) {	
			...
			nemoRequest.init( 
			{
				...
				url: 'masssms/write/masssms.do?method=listTestSMSResult&id=<%=id%>', 
				update: $('<%=id%>_grid_content'), // 완료후 content가 랜더링될 element
...
<%
	...
	if(method.equals("listTestSMSResult")) {
%>
			...
			<TR class="tbl_tr" systemNotifyList_id="<c:out value="${systemNotify.notifyID}"/>">
				...
				<TD class="tbl_td" align="left"><c:out value="${systemNotify.receiverPhone}" /></TD>
				<TD class="tbl_td"><c:out value="${systemNotify.wasSended}" /></TD>
				<TD class="tbl_td">[<c:out value="${systemNotify.smsCode}" />]<c:out value="${systemNotify.smsCodeMsg}" /></TD>
