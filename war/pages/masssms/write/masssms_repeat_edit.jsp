...
<div style="float:left" >
		<form id="<%=id%>_form" name="<%=id%>_form">
			...
			<tbody>
				<tr>
					<td class="ctbl ttd1" width="100px" align="left">대량SMS명</td>
					<td class="ctbl td"><c:out value="${masssmsInfo.masssmsTitle}"/></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="ctbl td"><%=ThunderUtil.descRepeatSendType(masssmsInfo.getRepeatSendType()) %></td>
				</tr>
				<c:if test="${masssmsInfo.repeatSendType=='2'}">
				<tr>
					<td colspan="10" class="ctbl line"></td>
				</tr>	
				<tr>
					...
					<td class="ctbl td">
					<input type="checkbox" class="notBorder" disabled <c:if test="${masssmsInfo.repeatSendWeekSun=='1'}">checked</c:if> value="1" />일&nbsp;&nbsp;
					<input type="checkbox" class="notBorder" disabled <c:if test="${masssmsInfo.repeatSendWeekMon=='2'}">checked</c:if> value="2" />월&nbsp;&nbsp;
					<input type="checkbox" class="notBorder" disabled <c:if test="${masssmsInfo.repeatSendWeekTue=='3'}">checked</c:if> value="3" />화&nbsp;&nbsp;
					<input type="checkbox" class="notBorder" disabled <c:if test="${masssmsInfo.repeatSendWeekWed=='4'}">checked</c:if> value="4" />수&nbsp;&nbsp;
					<input type="checkbox" class="notBorder" disabled <c:if test="${masssmsInfo.repeatSendWeekThu=='5'}">checked</c:if> value="5" />목&nbsp;&nbsp;
					<input type="checkbox" class="notBorder" disabled <c:if test="${masssmsInfo.repeatSendWeekFri=='6'}">checked</c:if> value="6" />금&nbsp;&nbsp;
					<input type="checkbox" class="notBorder" disabled <c:if test="${masssmsInfo.repeatSendWeekSun=='7'}">checked</c:if> value="7" />토&nbsp;&nbsp;
					</td>
				</tr>			
				</c:if>		
				<c:if test="${masssmsInfo.repeatSendType=='3'}">
				<tr>
					<td colspan="10" class="ctbl line"></td>
				</tr>	
				<tr>
					...
					<td class="ctbl td">
					<input type="text" readonly  size="3" value="<c:out value="${masssmsInfo.repeatSendDay}"/>" />일&nbsp;&nbsp;
					</td>
				</tr>			
				</c:if>							
				<tr>
				...
				<tr>
					...
					<td class="ctbl td"><c:out value="${masssmsInfo.repeatSendStartDate}"/> ~ <c:out value="${masssmsInfo.repeatSendEndDate}"/></td>					
				</tr>
...
<input type="hidden" name="eMasssmsID" value="<c:out value="${masssmsInfo.masssmsID}"/>" />	
	<table border="0" cellpadding="3" width="560px">
		...
		<tr>			
			...
			<div style="float:left;margin-top:2px">						
				<input type="text" id="eSendScheduleDateStart" name="eSendScheduleDateStart" size="10" style="cursor:pointer" readonly onclick="Calendar(event,this)" value="<%=DateUtils.getYearMonthDayStr(masssmsInfo.getRepeatSendStartDate()) %>"/>
			...
			<div style="float:left;margin-top:2px">						
				<input type="text" id="eSendScheduleDateEnd" name="eSendScheduleDateEnd" size="10" style="cursor:pointer" readonly onclick="Calendar(event,this)" value="<%=DateUtils.getYearMonthDayStr(masssmsInfo.getRepeatSendEndDate()) %>"/>
			...
			<%if(isAdmin.equals("Y") || masssmsInfo.getUserID().equals(userID)){ %>
...
$('<%=id%>').deleteScheduleCheck = function(){
	...
	nemoRequest.init( 
	{
		...
		, url: 'masssms/write/masssms.do?method=deleteRepeatScheduleByChecked&id=<%=id%>'
...
$('<%=id%>').deleteScheduleDate = function(){
	...
	nemoRequest.init( 
	{
		...
		, url: 'masssms/write/masssms.do?method=deleteRepeatScheduleByDate&id=<%=id%>'
		, update: $('<%=id%>_grid_content') // 완료후 content가 랜더링될 element
		, onSuccess: function(html,els,resHTML) {
...
$('<%=id%>').list = function (forPage) {
	...
	nemoRequest.init( 
	{
		...
		url: 'masssms/write/masssms.do?method=listRepeatSchedule&id=<%=id%>&masssmsID=<%=masssmsID%>', 
		update: $('<%=id%>_grid_content'), // 완료후 content가 랜더링될 element
...
%>	
	...
	<c:forEach items="${masssmsRepeatSchedule}" var="masssmsList">	
	<TR class="tbl_tr" attachedList_id="<c:out value="${masssmsList.scheduleID}"/>">	
		
		<TD class="tbl_td" align="center">
			<input type="hidden" name="eMasssmsID" value="<c:out value="${masssmsList.masssmsID}"/>" />			
			<input type="checkbox" id="eScheduleID" name="eScheduleID" value="<c:out value="${masssmsList.scheduleID}" />" />
		</TD>	
		<TD class="tbl_td"><c:out value="${masssmsList.sendScheduleDate}"/></TD>
		<TD class="tbl_td"><c:out value="${masssmsList.sendStartTime}"/></TD>		
		<TD class="tbl_td" align="center">
		...
		<TD class="tbl_td" align="center">
			<a href="javascript:$('<%=id%>').showStatistic(<c:out value="${masssmsList.masssmsID}"/>, <c:out value="${masssmsList.scheduleID}"/>)"><img src="images/chart_bar.png" title="통계보기 ">
		</TD>
	</TR>
	</c:forEach>
