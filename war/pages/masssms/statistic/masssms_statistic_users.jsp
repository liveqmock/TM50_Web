...
<div class="search_wrapper" style="width:870px">
	...
	<div class="btn_green"><a href="javascript:$('<%=id%>').loadContent('masssmsStatisticUsersBar','send')" style="cursor:pointer"><span>막대 그래프 조회</span></a></div>	
	<div class="btn_b"><a href="javascript:$('<%=id%>').loadContent('masssmsStatisticUsersPie','send')" style="cursor:pointer"><span>원 그래프 조회</span></a></div>
...
<div id="<%=id%>List" >
		...
		<tr>
			...
			<th style="height:30px;width:100px">대기통수</th>
		</tr>
		</thead>
...
$('<%=id%>').list = function () {	
	...
	nemoRequest.init( 
	{
		...
		url: 'masssms/statistic/masssms.do?method=masssmsStatisticUsers&id=<%=id%>', 
...
$('<%=id%>').loadContent = function(method, key) {
	...
	nemoRequest.init( 
		{
			url: 'pages/masssms/statistic/masssms_statistic_users_proc.jsp?method='+method+'&sUserID='+frm.sUserID.value,
	...
	nemoWindow(
			{
				...
				type: 'modal',
				loadMethod: 'xhr',
				contentURL: 'pages/masssms/statistic/masssms_report_month_proc.jsp?id=<%=id%>&method=searchuser&selectusers='+$('<%=id%>_sform').sUserID.value
			}
		);
}
//************************************************************
...
$('<%=id%>').excelDown = function() {
	var frm = $('<%=id%>_sform');
	location.href = "pages/masssms/statistic/masssms_statistic_users_excel.jsp?sYear="+frm.sSendedYear.value+"&sMonth="+frm.sSendedMonth.value+"&sUserID="+frm.sUserID.value+"&sGroupID="+frm.sGroupID.value;
}
