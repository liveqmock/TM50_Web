...
$('<%=id%>').list = function(forPage) {
	...
	page = 'masssms/statistic/masssms.do?method=masssmsReportMonthList&curPage='+$('<%=id%>').searchForm.curPage.value;
...
$('<%=id%>').getList = function(title, state) {
	...
	$('<%=id%>_dash_box_title').innerHTML = '<h2>'+title+' SMS 리스트</h2>';
	$('<%=id%>').list();
}
...
$('<%=id%>').loadContent = function() {
	page = 'masssms/statistic/masssms.do?method=masssmsReportMonthBasic'
	nemoRequest.init( 
		{
			url: page, 
			...
			onSuccess: function(html,els,resHTML,scripts) {
				...
				$('<%=id%>_dash_box_title').innerHTML = '<h2>총 발송 SMS 리스트</h2>';
...
$('<%=id%>').excelDown = function() {
	var frm = $('<%=id%>_sform');
	location.href = "pages/masssms/statistic/masssms_report_month_excel.jsp?sYear="+frm.sYear.value+"&sMonth="+frm.sMonth.value+"&sUserID="+frm.sUserID.value+"&sState="+frm.sState.value+"&sGroupID="+frm.sGroupID.value;
}
