...
<%
	...
	String listTitle = "대량 SMS 발송 리스트";
    ...
    MassSMSReportMonth massMailReportMonthTotalInfo = service.massSMSReportMonthTotalInfo(year, month, userID, groupID, userInfo);
    MassSMSReportMonth massMailReportMonthSendInfo = service.massSMSReportMonthSendInfo(year, month, userID, groupID, userInfo);
	...
	List<MassSMSList> masssmslist = service.massSMSReportMonthList(userInfo, 1, 500, searchMap);
    ...
    try {
        ...
        label = new Label(0,3,"총 대량SMS 수", defaultFormat);
        sheet.addCell(label);
        label = new Label(3,3,massMailReportMonthTotalInfo.getTotalCount()+"", dataFormat);
        sheet.addCell(label);
        label = new Label(6,3,"임시 저장", defaultFormat);
        sheet.addCell(label);
        label = new Label(9,3,massMailReportMonthTotalInfo.getWriteCount()+"", dataFormat);
        sheet.addCell(label);
        ...
        sheet.addCell(label);
        label = new Label(3,4,massMailReportMonthTotalInfo.getAppReadyCount()+"", dataFormat);
        sheet.addCell(label);
        label = new Label(6,4,"발송 대기", defaultFormat);
        sheet.addCell(label);
        label = new Label(9,4,massMailReportMonthTotalInfo.getReadyCount()+"", dataFormat);
        sheet.addCell(label);
        ...
        sheet.addCell(label);
        label = new Label(3,5,massMailReportMonthTotalInfo.getSendingCount()+"", dataFormat);
        sheet.addCell(label);
        label = new Label(6,5,"발송 완료", defaultFormat);
        sheet.addCell(label);
        label = new Label(9,5,massMailReportMonthTotalInfo.getSendFinishCount()+"", dataFormat);
        sheet.addCell(label);
        ...
        sheet.addCell(label);
        label = new Label(3,6,massMailReportMonthTotalInfo.getSendingStopCount()+"", dataFormat);
        sheet.addCell(label);
        label = new Label(6,6,"발송 에러", defaultFormat);
        sheet.addCell(label);
        label = new Label(9,6,massMailReportMonthTotalInfo.getErrCount()+"", dataFormat);
        sheet.addCell(label);
        ...
        sheet.addCell(label);
        label = new Label(3,9,massMailReportMonthSendInfo.getSendTotal()+"", dataFormat);
        sheet.addCell(label);
        label = new Label(6,9,"성공 통수", defaultFormat);
        sheet.addCell(label);
        label = new Label(9,9,massMailReportMonthSendInfo.getSuccessTotal()+"", dataFormat);
        sheet.addCell(label);
        ...
        sheet.addCell(label);
        label = new Label(3,10,massMailReportMonthSendInfo.getFailTotal()+"", dataFormat);
        sheet.addCell(label);
        label = new Label(6,10,"대기 통수", defaultFormat);
        sheet.addCell(label);
        label = new Label(9,10,massMailReportMonthSendInfo.getReadyTotal()+"", dataFormat);
        sheet.addCell(label);
/*        
        ...
        sheet.addCell(label);
        label = new Label(3,11,massMailReportMonthSendInfo.getClickTotal()+"", dataFormat);
        sheet.addCell(label);
        label = new Label(6,11,"수신 거부 통수", defaultFormat);
        sheet.addCell(label);
        label = new Label(9,11,massMailReportMonthSendInfo.getRejectcallTotal()+"", dataFormat);
        sheet.addCell(label);
*/        
        ...
        sheet.addCell(label);
        label = new Label(1,14,"대량SMS명", defaultFormat);
        sheet.addCell(label);
        ...
        int i =0;
        String StateType = "";
        for(MassSMSList masssms : masssmslist){
        	i++;
        	sheet.mergeCells(1, 14+i, 3, 14+i);
     	    ...
     	    label = new Label(0,14+i,masssms.getMasssmsID()+"" , dataFormat);
            sheet.addCell(label);
            label = new Label(1,14+i,masssms.getMasssmsTitle(), dataFormat);
            sheet.addCell(label);
            label = new Label(4,14+i,masssms.getUserName(), dataFormat);
            sheet.addCell(label);
            label = new Label(6,14+i,masssms.getSendScheduleDate(), dataFormat);
            sheet.addCell(label);
            label = new Label(8,14+i,masssms.getSendCount()+"", dataFormat);
            sheet.addCell(label);
             
             if(masssms.getState().equals("00")){
             	StateType = "임시저장중";
             }
             if(masssms.getState().equals("10")){
             	StateType = "승인대기중";
             }
             if(masssms.getState().equals("11")){
             	StateType = "발송준비대기중";
             }
             if(masssms.getState().equals("12")){
             	StateType = "발송준비중";
             }
             if(masssms.getState().equals("13")){
             	StateType = "발송준비완료";
             }
             if(masssms.getState().equals("14")){
             	StateType = "발송중";
             }
             if(masssms.getState().equals("15")){
             	StateType = "발송완료";
             }
             if(masssms.getState().equals("16")){
             	StateType = "오류자재발송중";
             }
             if(masssms.getState().equals("22")){
             	StateType = "발송준비중에러";
             }
             if(masssms.getState().equals("24")){
             	StateType = "발송중에러";
             }
             if(masssms.getState().equals("26")){
             	StateType = "오류자재발송중에러";
             }
             if(masssms.getState().equals("32")){
             	StateType = "발송준비중정지";
             }
             if(masssms.getState().equals("34")){
             	StateType = "발송일시정지";
             }
             if(masssms.getState().equals("44")){
             	StateType = "발송정지";
             }
