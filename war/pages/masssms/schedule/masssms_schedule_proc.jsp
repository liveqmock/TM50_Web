...
<%@ page import="web.masssms.schedule.model.ScheduleCalendar"%>
			...
			for (int i = 1; i <= numberOfDays; i++)
			{
				...
				for (  int j = 0 ; j < scheduleList.size() ; j ++ )
				{
					...
					{	size_mail++;
						...
						String title = "";
						String state = "";
						if(scheduleList.get(j).getMasssmsTitle().length() >= 6)
							title = scheduleList.get(j).getMasssmsTitle().substring(0,6)+"..";
						else
							title = scheduleList.get(j).getMasssmsTitle();
						/*
						...
						*/
						...
						if(size_mail <5)
						{
							...
							{//내 캠페인							
								...
								cell_html.append(scheduleList.get(j).getMasssmsTitle() + "(" + scheduleList.get(j).getSendScheduleDate().substring(11,16)+" 발송"+scheduleList.get(j).getTargetTotalCount()+"통)'>");
						...
						else
						{
							...
							{							
								...
								table_html.append(scheduleList.get(j).getMasssmsTitle() + "(" + scheduleList.get(j).getSendScheduleDate().substring(11,16)+" 발송"+scheduleList.get(j).getTargetTotalCount()+"통)'>");
...
%>
	...
	<div style="margin-bottom:10px;width:100%">
		...
		<form id="<%=id%>_form" name="<%=id%>_form" method="post">
			...
			<tbody>
			<tr>
				<td class="ctbl ttd1" width="80px">대량SMS명</td>
				<td class="ctbl td"><%=s.getMasssmsTitle()%></td>
			</tr>
			<tr>
	...
	if( LoginInfo.getAuth_sub_menu(request).indexOf("21")>-1 || LoginInfo.getIsAdmin(request).equals("Y") ){%>
