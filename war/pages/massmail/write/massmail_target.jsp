...
$('<%=id%>').list = function ( forPage ) {	
	...
	nemoRequest.init( 
	{
		...
		url: 'massmail/write/massmail.do?method=targetList&id=<%=id%>', 
...
%>
	...
	<TR class="tbl_tr" targetList_id="<c:out value="${targeting.targetID}"/>">
		...
		<TD class="tbl_td" align="center">
			...
			<c:if test="${targeting.targetType=='6'}">		
				회원정보UI	
			</c:if>
		</TD>
		<TD class="tbl_td"><a href="javascript:$('<%=id%>').previewWindow(<c:out value="${targeting.targetID}"/>,3)" title="미리보기"><fmt:formatNumber value="${targeting.targetCount}" type="number"/></a></TD>	
