...
%>
	...
	<input type="hidden" id="matrixX" name="matrixX" value="<%=matrixX %>">
	<input type="hidden" id="matrixY" name="matrixY" value="<%=matrixY %>">
	<input type="hidden" id="ranking" name="ranking" value="<%=ranking %>">
	...
	<div class="search_wrapper" style="clear:both;width:800px">
		<div class="right">
	...
	<div style="clear:both;width:400px">
		...
		<thead>
			<tr>
				...
				<th style="height:30px;width:400px">응답(주관식)</th>
				<th style="height:30px;width:200px">응답시간</th>
			</tr>
		</thead>
	...
	$('<%=id%>').list = function ( forPage ) {
		...
		nemoRequest.init( 
		{
			...
			url: 'massmail/statistic/massmail.do?method=massmailPollPreview&id=<%=id%>', 
...
//설문리스트 
if(method.equals("pollpreviewlist")) {
...
%>
	...
	<c:forEach items="${pollPersonPreviewList}" var="personPreview">
		<TR class="tbl_tr" >
			...
			<TD class="tbl_td" align="left"><c:out value="${personPreview.responseText}" escapeXml="false"/></TD>
	...
	$('<%=id%>').personPreviewDown = function() {
		location.href = "pages/massmail/statistic/massmail_poll_preview_excel.jsp?massmailID=<%=massmailID%>&scheduleID=<%=scheduleID%>&pollID=<%=pollID%>&iTotalCnt=<%=iTotalCnt%>&questionID=<%=questionID%>&exampleID=<%=exampleID%>&matrixX=<%=matrixX%>&matrixY=<%=matrixY%>&ranking=<%=ranking%>";
