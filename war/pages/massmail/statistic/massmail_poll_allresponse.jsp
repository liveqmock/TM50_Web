...
//설문응답자보기 
//****************************************************************************************************/
if(method.equals("pollAllResponse")) {	
...
%>
	...
	<div style="clear:both;width:400px">
		...
		<thead>
			<tr>
				...
				<th style="height:30px;width:270px">응답시간</th>
			</tr>
		</thead>
	...
	$('<%=id%>').list = function ( forPage ) {
		...
		nemoRequest.init( 
		{
			...
			url: 'massmail/statistic/massmail.do?method=massmailPollAllResponse&id=<%=id%>', 
...
//설문리스트 
if(method.equals("pollAllResponseList")) {
...
%>
	...
	<c:forEach items="${pollPersonPreviewList}" var="personPreview">
		<TR class="tbl_tr" >
			<TD class="tbl_td"><a href="javascript:$('<%=id%>').showPollDetailView('${personPreview.sendID}','${personPreview.email}')"><c:out value="${personPreview.email}"/></a></TD>			
	...
	<script type="text/javascript">
		...
		$('<%=id%>').personPreviewDown = function() {			
			location.href = "pages/massmail/statistic/massmail_poll_allresponse_excel.jsp?massmailID=<%=massmailID%>&scheduleID=<%=scheduleID%>&pollID=<%=pollID%>&iTotalCnt=<%=iTotalCnt%>";
			
		}

		$('<%=id%>').showPollDetailView = function(sendID,email){
			nemoWindow(
					{
						'id': '<%=id%>_pollDetailView',
						...
						type: 'modal',
						loadMethod: 'xhr',
						contentURL: 'massmail/statistic/massmail.do?id=<%=id%>_pollDetailView&method=massmailPollAnswerView&pollID=<%=pollID%>&massmailID=<%=massmailID%>&scheduleID=<%=scheduleID%>&sendID='+sendID+'&email='+email
					}
				);
		}
