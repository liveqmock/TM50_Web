...
<form id="<%=id%>_sform" name="<%=id%>_sform">
	<div class="search_wrapper" style="width:97%;">
		...
		<div class="right">
			<div class="my"></div><div class="text"><strong>내 캠페인</strong></div>
			<div class="mygroup"></div><div class="text"><strong>내 그룹 캠페인</strong></div>
			<div class="outer"></div><div class="text"><strong>다른 그룹 캠페인</strong></div>
		</div>
		
	</div>
	<div style="clear:both;width:97%">
		<div class="text" style="padding-left:8px">
			<img src="images/tag_blue.png"> 날짜를 클릭하면 대량메일을 작성할 수 있습니다.  
		</div>
	</div>
</form>
<div style="clear:both;width:97%">
	...
	nemoWindow(
		{
			...
			noOtherClose: true,
			loadMethod: 'xhr',
			contentURL: 'massmail/write/massmail.do?step=listRepeat&id=massmailRepeat&eSendScheduleDateStart='+send+'&eSendScheduleDateEnd='+send
		}
	);
	...
	nemoWindow(
		{
			...
			noOtherClose: true,
			loadMethod: 'xhr',
			contentURL: 'massmail/write/massmail.do?step=write&id=massmailWrite&eSendScheduleDateStart='+send
		}
	);
...
$('<%=id%>').list = function ( forPage ) {	
	...
	nemoRequest.init( 
	{
		...
		url: 'massmail/schedule/schedule.do?method=list&id=<%=id%>&groupID='+frm.sGroupID.value, 
